...
namespace SharpConfigProg.Service
{
    public class ConfigService
    {
        ...
        private readonly FileService.IYamlOperations yamlOperations;
        

        public string ConfigFilePath { get; }
        public Dictionary<string, object> Paths
        {
            get; 
            private set;
        }
            ...
            FileService fileService)
        {
            ...
            ConfigFilePath = MoveDirectoriesUp(binPath, 3) +
                "/" + "Config" + "/" + "paths.cfg";
            CreateMissingDirectories(ConfigFilePath);

                this.fileService = fileService;
            //this.repoService = repoService;
        }

        public List<string> GetRepoRootPaths()
        {
            var repoRootPaths = (Paths["repoRootPaths"] as List<object>)
        ...
        public void PrepareOnlyRepoRootPaths()
        {
            var repoRootPaths = GetRepoPaths();
            Paths = new Dictionary<string, object>()
            {
                { "repoRootPaths", repoRootPaths},
            };
            Paths.ToList().ForEach(x => Console.WriteLine(x.Key + ": " + x.Value));
            yamlOperations.SerializeToFile(ConfigFilePath, Paths);
        }

        public List<object> GetRepoPaths()
        {
            ...
            public void PrepareForWidner()
        {
            ...
            var isServer = IsServer(computerName);
