...
namespace SharpRepoServiceCoreProj
{
    public class RepoServiceMethods
    {
        ...
        private readonly string nameFileName;
        ...
        private List<string> rootPathsList;
           ...
           LocalInfo localInfo)
        {
            ...
            rootPathsList = localInfo.LocalRootPaths;
            contentFileName = "lista.txt";
            nameFileName = "nazwa.txt";
            repoConfigName = "repoConfig.txt";

            allRepoPathsList = new List<string>();
            
            InitializeRepoPathsList();
        }

        public RepoServiceMethods(FileService fileService)
        {
            this.fileService = fileService;
        }
        ...
        public string GetConfigText((string Repo, string Loca) address)
        {
            ...
            var nameFilePath = itemPath + slash + nameFileName;
        ...
        private void InitializeRepoPathsList()
        {
            foreach (var rootPath in rootPathsList)
            {
                var folders = Directory.GetDirectories(rootPath).Select(x => CorrectPath(x));
            ...
            string name)
        {
            ...
            var nameFilePath = elemPath + slash + nameFileName;
        ...
        {
            var foundList = allRepoPathsList.Where(x => Path.GetFileName(x) == repo);
        ...
        public string GetLocalName((string repo, string loca) address)
        {
            ...
            var path = repoPath + "/" + address.loca + "/" + nameFileName;
        ...
        {
            var path = elemPath + "/" + nameFileName;
            var name = File.ReadAllLines(path).First();
            return name;
        }
